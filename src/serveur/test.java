...
public class test implements javax.jms.MessageListener{
    ...
    private javax.jms.MessageProducer qReceiver = null;
    ...
    private void configurerProducteur() throws JMSException, NamingException{
    	...
    	sendSession = connect.createSession(true,-1);// javax.jms.Session.AUTO_ACKNOWLEDGE);
    ...
    private void produire() throws JMSException{
    	for (int i=1;i<=10;i++){
    		...
    		sender.send(mess);
    		System.out.println("	Production de mess: "+ mess);
    		if (i==5) sendSession.commit(); // commite les msgs emis aux tours 1,2..,5
    		if (i==10) sendSession.rollback(); // annule l'envoi des msgs 6,...10. 
    	}
    }
